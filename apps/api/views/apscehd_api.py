...
"""api_view:定义此接口可以使用的method
#permission_required:来限定view函数只有在User具有相应权限的情况下才能被访问
表示有这个权限才能进行访问，不然就抛出异常"""
...
def node_list(request,format=None):
    ...
    序列化Sched_Node这个模型，先获取这个模型的所有的数据，
    然后序列化它，many参数表示序列化多个字段，应用于多对多关系
    返回数据
    """
