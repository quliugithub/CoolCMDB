...
class PageConfig(CursorPagination):
    """
    加密分页
    page_size_query_param：如果设置，则这是一个字符串值，指示允许客户端基于每个请求设置页面大小的查询参数的名称。
    默认为None，表示客户端可能无法控制请求的页面大小
    max_page_size:后台控制显示的最大记录条数，防止前台输入的条数过大
    page_size:表示页面大小的数值
    """
...
class BusinessTreeSerializer(serializers.ModelSerializer):
    """
    SerializerMethodField:在使用django restframework serializer
    序列化在django中定义的model时，
    有时候我们需要额外在serializer中增加一些model中没有的字段
    read_only:表示该字段仅向前端页面输出输出数据，默认为False
    required:表示该字段在反序列化时，必须输入默认为True
    paths，icon，last_node,mannage_name,env_name这些字段在model中并没有，这些
    字段没有在数据库维护，在需要的时候再取
    """
...
class CabinetSerializer(serializers.ModelSerializer):
    """
    source:这里可以指定显示的具体字段,如果不指定就显示主键值
    many:
    """
